...
/**
 * Creates a form, which is shown when new application is going to be created.
 ...
 *
 */
public class PopupAddNewApp extends FormPopup  implements AsyncEventListener{
	 ...
	 * 
	 */
	public PopupAddNewApp(Template template) {
		...
		tb = new TextBox();
		tb.setText(template.getProperties().get(Property.RELATED_URL).getStringValue());
	...
	public void onSuccessfullyFilled() {
		...
		savePageAsComponent(((TextBoxBase)uiElements.get(UI_ID).getElement()).getText(), 
				...
				PopupAddNewApp.this.template.getProperties().get(Property.APP_ID).getStringValue(),
				PopupAddNewApp.this.template.getProperties().get(Property.ID).getStringValue());
	}
